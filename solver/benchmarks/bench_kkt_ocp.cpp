...
static smf::CscLower build_traj_opt_hessian(int N_steps, int nx, int nu) {
    ...
    auto add_entry = [&](int row, int col, double val) {
        ...
        if (row >= col) {
            rc_pairs.push_back({static_cast<smf::Int>(row),
                                static_cast<smf::Int>(col)});
            vals.push_back(val);
        }
    };
...
static int bench_traj_opt(int N_steps, int nx, int nu, int N_iter) {
    ...
    for (int iter = 0; iter < N_iter; ++iter) {
        ...
        if (iter == N_iter - 1) {
            ...
            for (int i = 0; i < n; ++i) {
                double r = ax[static_cast<std::size_t>(i)] -
                           b_orig[static_cast<std::size_t>(i)];
                num += r * r;
                den += b_orig[static_cast<std::size_t>(i)] *
                       b_orig[static_cast<std::size_t>(i)];
            }
