    ...
    Info& info)
{
    ...
    // For each supernode, collect its row indices from the original CSC pattern.
    // row_indices = sorted union of all row_idx entries in columns col_start..col_end-1
    for (Int s = 0; s < nsn; ++s) {
        ...
        // Add diagonal entries (the pivot columns themselves)
        for (Int j = sn.col_start; j < sn.col_end; ++j)
            seen[static_cast<std::size_t>(j)] = true;

        // Add off-diagonal entries from the original pattern
        for (Int j = sn.col_start; j < sn.col_end; ++j) {
