    ...
    const EliminationTree& etree)
{
    ...
    for (Int j = 0; j < n; ++j) {
        ...
        if (!new_snode) {
            ...
            new_snode = (par_prev != j)
                     || (nchildren[static_cast<std::size_t>(j)] != 1)
    ...
    Int nemin)
{
    ...
    while (changed) {
        changed = false;
        for (Int s = 0; s < nsn; ++s) {
            if (supernodes[static_cast<std::size_t>(s)].col_start < 0) continue;
            const Int p = supernodes[static_cast<std::size_t>(s)].parent;
            if (p < 0) continue;
            if (supernodes[static_cast<std::size_t>(p)].col_start < 0) continue;
