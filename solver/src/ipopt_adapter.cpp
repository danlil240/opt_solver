...
namespace smf
{
    ...
    SmfLinearSolver::SmfLinearSolver(Control ctrl) : ctrl_(ctrl)
    {
    ...
    void SmfLinearSolver::build_pattern(int n, int nnz, const int* irn, const int* jcn)
    {
    ...
    {
        // ak_->cleaned stores the ORIGINAL-ordered matrix (symbolic_analysis.cpp
        // line: keep->cleaned = std::move(cleaned.clean)).  factor() permutes it
        // internally via permute_lower_csc*(cleaned, iperm).  Therefore we must
        // search by ORIGINAL (old_j, old_i) indices — no iperm involved here.
        ...
        for (int old_j = 0; old_j < n; ++old_j)
        {
                 ...
                 p < mat_.col_ptr[static_cast<std::size_t>(old_j) + 1]; ++p)
            {
                ...
                // Binary-search for old_i in ak_->cleaned column old_j.
    ...
    Ipopt::ESymSolverStatus SmfLinearSolver::InitializeStructure(int n, int nnz, const int* irn, const int* jcn)
    {
        n_ = n;
        neg_evals_ = 0;
        info_ = Info{};
    ...
    Ipopt::ESymSolverStatus SmfLinearSolver::MultiSolve(bool new_matrix,
                                                        const int* /*irn*/,
                                                        const int* /*jcn*/,
                                                        const double* values,
                                                        int nrhs,
                                                        ...
                                                        int numberOfNegEVals)
    {
        ...
        if (nrhs > 0)
        {
            const int rc = solver_.solve(fk_, ctrl_, info_, rhs_vals, n_, nrhs, SolveJob::Full);
            if (rc != 0)
            {
                return Ipopt::SYMSOLVER_FATAL_ERROR;
            }
    ...
    int SmfLinearSolver::NumberOfNegEVals() const { return neg_evals_; }
