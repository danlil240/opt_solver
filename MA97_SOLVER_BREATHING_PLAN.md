...
- **Active phase:** Post-plan hardening — MA97 plugin ABI fix ✅ DONE; SPD tiny-benchmark triage ✅ DONE; trajectory same-KKT MA27 comparison ✅ DONE; trajectory solve traversal hardening ✅ PARTIAL; strict cold one-shot solve parity ⚠️ NOT ACHIEVED.
- **Build state:** focused regression tests green after cold-solve timing investigation: `Supernode`, `IpoptAdapter`, `SolveResidual`, `OcpKktRegression`, `IndefLarger` all passed. Last full-suite baseline before this wave was 42/42 green.
...
- **Trajectory state:** same-matrix capture/compare found the conservative 0.01 indefinite `pivot_u` floor was a real integration defect: on captured KKT `kkt_0002`, smf residual was `1.78e-5` at `u=0.01` vs `1.01e-12` at IPOPT's requested `u=1e-8`. The floor was removed; the current installed `libsmf_ma97.so` honors IPOPT's `ctrl->u`. Full trajectory still returns `Solved To Acceptable Level` but remains slow: 1742 IPOPT iterations / 11.391 s.
- **Solve-path state:** sibling-preserving contiguous amalgamation still gives captured KKT structure `nsteps=1400`, `width1=898`, `width2=262`, `width3plus=240`, `maxfront=21`, `factor_values=23665` at `nemin=8`. Beta instrumented first-vs-warm solves and fixed the benchmark so optional smf profiling no longer runs before MA27's displayed one-shot solve. Final strict one-shot solve remained slower: `kkt_0002` smf/MA27 `0.062/0.050 ms`; `kkt_0020` `0.062/0.054 ms`. Profiled smf full solve in the final fair-order run was `~0.049-0.050 ms`, with first full `~0.061 ms`.
- **Next focus:** strict one-shot smf solve did not honestly beat MA27. Tried and reverted non-wins: sequential-write permutation, cached inverse-D entries, no-shrink local scratch, and factor-time solve-cache touch. Remaining measured reason is cold/cache and traversal overhead in the first full forward+diag+back call after factorization; further gains likely need a deeper solve-layout redesign rather than local scratch or timing-method tweaks.
---
